...
void robotThread(void *params){
	...
	*/
	...
	lastRT.frontSonar = robot.getClosestSonarRange(-45,45);
	lastRT.backSonar = robot.getClosestSonarRange(135,225);
	curRT = lastRT;

	while(!myEndProgram){
		count++;
		int frontDistance = robot.getClosestSonarRange(-45,45);
		int backDistance = robot.getClosestSonarRange(135,225);
		...
		if(frontBuffer->read==false && frontBuffer->size>0){
			...
			for(int i=0;i<frontBuffer->size;i++){
				...
				if(frontBuffer->data[i].data == FORWARDS && frontBackIsActive){
					...
					curRT.frontSonar = robot.getClosestSonarRange(-45,45);
					curRT.backSonar = robot.getClosestSonarRange(135,225);
