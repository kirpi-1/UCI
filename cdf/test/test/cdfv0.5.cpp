...
bool cdf::FrameInfo::setData(float *in, unsigned int xlen, unsigned int ylen){
	if(this->data!=NULL)
		delete[] data;
	data = new float[xlen*ylen];
	memcpy(this->data,in,xlen*ylen*sizeof(float));
	return true;
}
...
void cdf::CDFSession::printFrame(int idx){
...
int cdf::CDFSession::writeFrame(FrameInfo &fi){
	//cdf::printFrame(&fi);
	return cdf::writeFrame(file,&fi);
}
...
int cdf::writeASCIIHeader(fstream &fout, SessionInfo &si, VariableHeader &vh){	
	...
	//Action filename
	sprintf_s(buffer,"Action filename: %s\r\n",vh.actionFilename.c_str());
	header = header+buffer;
	//number of actions
	sprintf_s(buffer,"Num Actions: %u\r\n",si.numActions);
	header = header+buffer;
	//the actions
	for(int i=0;i<si.numActions;i++){
...
int cdf::writeVariableHeader(fstream &fout, SessionInfo &si, VariableHeader &vh){
	...
	fout.write(vh.coiFilename.c_str(),FILENAME_SIZE);
	fout.write(vh.actionFilename.c_str(),FILENAME_SIZE);
	//**************write the names***********
	//COIs
	for(unsigned int i=0;i<si.numCOI;i++)	
