...
void cdf::Frame::initialize(int idx, unsigned int hs, unsigned int x, unsigned int y){
	...
	setHeaderSize(hs);
	setFrameSize(x, y);
	memset(m_header,0,m_allocatedHeaderSize);
...
bool cdf::Frame::setHeader(void *in, unsigned int size){
	if(size > m_allocatedHeaderSize){
		delete[] m_header;
		m_allocatedHeaderSize = size;
		m_header = new BYTE[m_allocatedHeaderSize];
	}
	else{
		clearHeader();
	}
...
bool cdf::Frame::setHeaderSize(unsigned int s){
	if(s > m_allocatedSize){
		if(m_header != NULL)
			delete[] m_header;
		m_header = new BYTE[s];
		m_allocatedHeaderSize = s;
	}
	m_headerSize = s;
	return true;
}
...
int cdf::CDFSession::open(){
	...
	sprintf_s(buffer,FILENAME_SIZE,"%d-%d-%d %d.%d Subject %d",d.year,d.month,d.day,d.hour,d.minute,subnum);
	return open(buffer);
}
...
int cdf::CDFSession::writeFrame(){
	...
	file.write((char *)curFrame.header(),sizeof(char)*curFrame.headerSize());
