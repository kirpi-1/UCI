...
void zANT::registerMessageWindow(){
	...
	m_hWnd = CreateWindowEx
        ...
        TEXT("Main zANT Window"), //window name
...
void zANT::setFileName(char f[]){
	...
	strcpy_s(m_filename,DEFAULT_FILENAME_SIZE,f);
}
...
bool zANT::startNewFile(){
	...
	char filename[DEFAULT_FILENAME_SIZE+4];
	if(m_filename[0]!=NULL){
		sprintf_s(filename,DEFAULT_FILENAME_SIZE+4,"%s.cdf",m_filename);	
	}
	else{
		sprintf_s(filename,DEFAULT_FILENAME_SIZE+2,"part%02d.cdf",m_filePartIdx);
	}
	cdfs.open(filename,true);	
	//cdf::getVariableHeaderSize(cdfs.session);
	sprintf_s(m_msg,MSG_SIZE,"starting new file \"%s\", variable header size %d",filename,cdfs.session.variableHeaderSize);
...
bool zANT::closeFile(){	
	...
	cdfs.close();
	m_fileOpen = false;
	sprintf_s(m_msg,MSG_SIZE,"wrote %d frames",m_nframe);
	verboseMessage();
	m_filePartIdx++;
	return true;
}
...
void zANT::toggleTestData(){
	...
	(m_useTestData)? onoff="Now":onoff="Stopped";
	printf("%s using test data\n",onoff);
	m_useNullData = false;
	m_useAmpData = false;
}

void zANT::toggleTestData(bool t){
	if(t!=m_useTestData)
		toggleTestData();
}

void zANT::toggleNullData(){
	char *onoff;
	m_useNullData = !m_useNullData;			
	(m_useNullData)? onoff="Now":onoff="Stopped";
	printf("%s using null data\n",onoff);
	m_useTestData = false;
	m_useAmpData = false;

}

void zANT::toggleNullData(bool t){
	if(t!=m_useNullData)
		toggleNullData();
}

void zANT::toggleAmpData(){
	char *onoff;
	m_useAmpData = !m_useAmpData;			
	(m_useNullData)? onoff="Now":onoff="Stopped";
	printf("%s using amplifier data\n",onoff);
	m_useTestData = false;
	m_useNullData = false;
}

void zANT::toggleAmpData(bool t){
	if(t!=m_useAmpData)
		toggleAmpData();
}
int zANT::getDataType(){
	if(m_useNullData)
		return 1;
	else if(m_useTestData)
		return 2;
	else if(m_useAmpData)
		return 3;
	else
		return 0;
}
...
{	
	if(m_useTestData){		
	...
	else if(m_useNullData){
		float zero[1]; 
		zero[0] = 0;
		cdfs.setFrameData(zero,1,1);
	}
	else if(m_useAmpData){
		...
		cdfs.setFrameData(testData,0,0);
