...
#define RANGE_FAR (1500)
#define RANGE_MED (1200)
#define RANGE_CLOSE (900)
#define RANGE_SHUTOFF (600)
using namespace std;
...
ConnectionStatus socketList[SOCKET_LIST_SIZE] = {FREE,FREE,FREE,FREE,FREE};
string socketListNames[] = {"EEG","Tablet","Save","AUX1","AUX2"};

const double sonarAngle = 90.0;
const int sonarFL = -45;
const int sonarFR = 45;
const int sonarBL = 180-45;
const int sonarBR = 180+45;
...
int main(int argc, char *argv[])
{
	...
	do{
		...
		ConnectionStatus bciStatus = socketList[BCI_INDEX];
		ConnectionStatus tabletStatus = socketList[TABLET_INDEX];
		LeaveCriticalSection(&cs);
		myConnectionClosed = false;
		if(useBCI && bciStatus==FREE){
			if(debugOutput)
			...
			bci->index = BCI_INDEX;
			EnterCriticalSection(&cs);
			socketList[bci->index] = CONNECTING;
			LeaveCriticalSection(&cs);
			_beginthread(handleTCPClient,0,(void *)(bci));		
		}

		if(useTablet && tabletStatus==FREE){
			if(debugOutput)
			...
			EnterCriticalSection(&cs);
			socketList[tablet->index] = CONNECTING;
			LeaveCriticalSection(&cs);
	...
	while(!quit){
		...
		for(int i=0;i<SOCKET_LIST_SIZE;i++){
			cout << ConnectionStatusNames[socketList[i]] << ", ";		
...
void robotThread(void *params){
	...
	Marvin = &robot;//so other threads can see him
	...
	*/
	...
	lastRT.getRobotTelemetry(robot,sonarAngle);
	curRT = lastRT;

	while(!myEndProgram){
		count++;
		//first, check with sonar if the robot got too close to anything
		int frontDistance = robot.getClosestSonarRange(sonarFL,sonarFR);
		int backDistance = robot.getClosestSonarRange(sonarBL,sonarBR);
		
		if(safeDrive){
			if(frontDistance<RANGE_SHUTOFF && robot.getVel()>0 || backDistance<RANGE_SHUTOFF&&robot.getVel()<0){
				teleop.space();
				teleop.setThrottle(0);
				if(debugOutput)
					printf("something is too close to sonar at %i, stopping\n",(frontDistance<backDistance)?frontDistance:backDistance);
				
			}
			else if(frontDistance<RANGE_CLOSE && robot.getVel()>0 || backDistance<RANGE_CLOSE && robot.getVel()<0){
				if(debugOutput)
			...
			else if(frontDistance<RANGE_MED && robot.getVel()>0 || backDistance<RANGE_MED && robot.getVel()<0){
				if(debugOutput)
					printf("something is too close to sonar, setting throttle to 1/4th speed of %u\n",teleop.getMaxThrottle()/4);		
				teleop.setThrottle(teleop.getMaxThrottle()/2);
			}
			else if(frontDistance<RANGE_FAR && robot.getVel()>0 || backDistance<RANGE_FAR && robot.getVel()<0){
				if(debugOutput)
					printf("something is too close to sonar, setting throttle to 1/2 speed of %u\n", teleop.getMaxThrottle()/2);		
				teleop.setThrottle(teleop.getMaxThrottle()*3/4);
			}
			else
		...
		if(frontBuffer->read==false && frontBuffer->size>0){
			...
			for(int i=0;i<frontBuffer->size;i++){
				...
				else if(frontBuffer->data[i].data == TOGGLE_SAFE_DRIVE){
					safeDrive = !safeDrive;
					cout << "safe drive " << ((safeDrive)?"on":"off") << endl;
				}
				else if(frontBuffer->data[i].data == POLL_TELEMETRY){						
					curRT.getRobotTelemetry(robot,sonarAngle);
					if(debugOutput){
						cout << "sending telemetry data...";
						cout << "\nvelocity: " << curRT.vel << endl
							 << "rotVlocity: " << curRT.rotVel << endl
							 << "voltage: " << curRT.voltage << endl
							 << "frontSonar: " << curRT.frontSonar << endl
							 << "backSonar: " << curRT.backSonar << endl;
						cout << "sending telemetry data to " << frontBuffer->data[i].myInt << "...\n";
					}
					int numbs = send(frontBuffer->data[i].myInt,reinterpret_cast<char*>(&curRT),sizeof(MyRobotTelemetry),0);
					if(numbs<sizeof(MyRobotTelemetry))
						cout << "couldn't send everything\n";					
					//set the last robot telemetry to what we just measured
					lastRT = curRT;
					if(debugOutput)
						cout << "done.\n";
...
void handleTCPClient(void* params){
	...
	if(clientSocket->clientSock<0 || clientSocket->clientSock == SOCKET_ERROR){
		...
		cout << socketListNames[clientSocket->index] << " is now connected at socket #"
		...
		while(numbr>0 && !disconnect){
			...
			else if(nError!=WSAEWOULDBLOCK)
			{
				...
				if(clientSocket->index==TABLET_INDEX){					
					if(!strcmp(buffer,"t")){
						MyRobotTelemetry rt;
						rt.getRobotTelemetry(*Marvin,sonarAngle);
						//printf("sending telemetry\n");
						send(clientSocket->clientSock,reinterpret_cast<char*>(&rt),sizeof(MyRobotTelemetry),0);
					}
					else{
						EnterCriticalSection(&cs);
				...
				if(isActive){
					//wait until the 
					int commandCount = parseCommands(buffer,numbr,clientSocket->clientSock);				
					EnterCriticalSection(&cs);
					if(debugOutput)
	...
	EnterCriticalSection(&cs);
	if(debugOutput)
		cout << "leaving " << socketListNames[clientSocket->index] << endl;
	int commandCount = 1;
	backBuffer->data[0].data = STOP;
	backBuffer->size = 1;
	backBuffer->read = false;
...
int parseCommands(char *buffer, int numbr, int clientSocket){
	...
	for(int i=0;i<numbr;i++){						
		if(buffer[i]=='f' || buffer[i] =='F')
			...
			backBuffer->data[commandCount].data = TOGGLE_SAFE_DRIVE;
		}
